...
#'
#' Identifies duplicate entries in a set of tabular data using one or more
#' columns, then labels matches by a unique match identifier.
#'
#' @param x A dataset in which duplicate entries will be identified
#' @param match_cols Column(s) that will be used to generate a search string for
#' duplicate identification. If more than one column is
#' specified, then the search string will be formed from the
#' concatenation of values from column specified in \code{match_cols}.
#' @param approx_match If set to \code{TRUE}, performs a duplicate search using
#' string distances rather than exact values.
#' @param string_dist When performing approximate matching, is is the distance
#' cutoff at which entries will be considered matched.
#' @param min_length The minimum length for values in \code{match_cols}
#' such that rows with \code{match_cols} shorter than the values specified by
#' this argument will be protected from identification as a duplicate.
#' This helps ensure that aberrant duplicates are not identified due to
#' very short or empty values within \code{match_cols}.
#' @param simplify_match If \code{TRUE}, modifies the matching string by
#' removing all alpha-numeric characters and setting the remaining string to
#' lower-case.
#' @return An updated version of \code{x}, with extra columns specifying the
#' final string used to search for duplicates (contained within a new column
#' called \code{matching_col}) and a unique identifier for each set of
#' duplicates (contained within a new column called \code{match_ID})
#' that that will share the same value for any identified duplicates.
#' @examples
#' \dontrun{
#' }
#' @export
...
#' @export
find_duplicates.default <- function(x, ...) {
    stop("x must be of class data.frame")
}

#' @export
find_duplicates.data.frame <- function(x,
                                       match_cols,
                                       approx_match = FALSE,
                                       string_dist = 10,
                                       min_length = 20,
...
#'
#' Takes output generated by \code{find_duplicates}, and returns a
#' \code{data.frame} containing only those entries of the original dataset
#' identified as duplicate entries.
#'
#' @param x The ouput of \code{find_duplicates}
...
#' may subsequently be spared when running \code{remove_duplicates}
#' by indicating their match IDs in \code{remove_duplicates}'s
...
#'
#' After running \code{find_duplicates}, duplicate sets of entries will contain
#' identical match IDs contained in the \code{match_ID} column. This
#' function will remove all but one entry from each set of duplicates. The
#' entry to be preserved is identified based on a combination of the
#' preferred source database (if specified in \code{db_pref}) and/or the entry
#' within the duplicate set with the least number of blank cells.
#'
#' @param x The output of \code{find_duplicates}
...
#' duplicate entries. These should be identified by manually inspecting
#' the output of \code{return_duplicates_df}.
#' @return An updated version of \code{x}, with duplicate entries removed.
#' @examples
#' \dontrun{
#' }
#' @export
...
#' @export
remove_duplicates.data.frame <- function(x, db_pref = NULL, ignore_IDs = NULL) {
